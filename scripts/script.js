...
let ship;
let clicks = {}; // storing mouse click inputs in an object
let enemies = [];
let score = 0;
...
class Ship {
    constructor(x, y, radius, HP) {
        ...
        this.radius = radius;
        this.HP = HP;
        this.bullets = []; //had to change this to public to be used for collision detection
    }
    shoot(dx, dy) {
    ...
    draw() {
        ...
        for (let i = 0; i < sides; i++) {
            ctx.lineTo(this.x - this.radius * Math.cos(vertAngle * i), this.y - this.radius * Math.sin(vertAngle * i));
        }
        ctx.closePath();
        ctx.stroke();
        ctx.fill();
...
function respawnEnemies(toSpawn) {
